...
const pro = list.filter((el,i)=>{
  if(list.indexOf(el)===i){
    return el
  }
})
return pro
...
function getFullNames(runners) {
  let d = []
  runners.forEach(x=>{
    d.push(x.last_name + ", " + x.first_name)
  })
  return d
}
...
function getRunnersByTShirtSize(runners,tShirtSize) {
 const shirtSize = runners.filter((a,b,c)=>{
    return a.shirt_size === tShirtSize
 })
 return shirtSize
}
 ...
 * etc
*/
function counterMakerWithLimit(threeLim) {
